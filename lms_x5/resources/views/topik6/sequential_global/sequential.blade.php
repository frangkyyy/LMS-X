    ...
    <div class="container">
        ...
        <div class="card mt-3">
            <div class="card-body">
                ...
                @if ($course && $course->id)
                    @php
                        $sequentialFiles = \App\Models\MDLFiles::where('course_id', $course->id)
                            ->where('learning_style', 'sequential')
                            ->where('topik', 'topik6')
                            ->whereIn('type', ['sequential-pdf', 'sequential-link'])
                            ->get();
                    @endphp
                ...
                @if ($sequentialFiles->count() > 0)
                    @foreach ($sequentialFiles as $file)
                        @if ($file->type === 'sequential-link')
                            <div class="mt-4">
                            ...
                            {{--                            </div>--}}
                            {{--                        @endif--}}
                        @elseif ($file->type === 'sequential-pdf')
                            <div class="mt-4">
