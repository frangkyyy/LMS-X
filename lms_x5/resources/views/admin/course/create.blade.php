        ...
        <div class="card mt-3">
            <div class="card-body">
                <div class="d-flex align-items-center mb-3">
                    ...
                    <h4 class="fw-bold mb-0">Add Course</h4>
                </div>
                ...
                <form action="{{ route('courses.store') }}" method="POST" id="courseForm" enctype="multipart/form-data">
                    ...
                    <div id="general-section">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('category') is-invalid @enderror" id="category" name="category" value="{{ old('category') }}" placeholder="Enter category (e.g., Technology, Science)">
                            @error('category')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div id="summary-section" style="display: none;">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('cpmk') is-invalid @enderror" id="cpmk" name="cpmk" rows="3">{{ old('cpmk') }}</textarea>
                            @error('cpmk')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div id="settings-section" style="display: none;">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('visible') is-invalid @enderror" id="visible" name="visible">
                                <option value="1" {{ old('visible', '1') == '1' ? 'selected' : '' }}>Show</option>
                                <option value="0" {{ old('visible') == '0' ? 'selected' : '' }}>Hide</option>
                            </select>
                            @error('visible')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <div class="d-flex gap-2">
                                <select class="form-select @error('start_day') is-invalid @enderror" name="start_day" id="start_day">
                                    @for ($i = 1; $i <= 31; $i++)
                                        <option value="{{ $i }}" {{ old('start_day', '1') == $i ? 'selected' : '' }}>{{ $i }}</option>
                                    @endfor
                                </select>
                                <select class="form-select @error('start_month') is-invalid @enderror" name="start_month" id="start_month">
                                    @foreach (['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'] as $month)
                                        <option value="{{ $month }}" {{ old('start_month', 'January') == $month ? 'selected' : '' }}>{{ $month }}</option>
                                    @endforeach
                                </select>
                                <select class="form-select @error('start_year') is-invalid @enderror" name="start_year" id="start_year">
                                    @for ($i = 2023; $i <= 2030; $i++)
                                        <option value="{{ $i }}" {{ old('start_year', '2023') == $i ? 'selected' : '' }}>{{ $i }}</option>
                                    @endfor
                                </select>
                                <input type="time" class="form-control @error('start_time') is-invalid @enderror" name="start_time" id="start_time" value="{{ old('start_time', '00:00') }}">
                                @error('start_day')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                @error('start_month')
                        ...
                        <div class="mb-3">
                            ...
                            <div class="d-flex align-items-center gap-2">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="enable_end_date" name="enable_end_date" {{ old('enable_end_date') ? 'checked' : '' }}>
                                    <label class="form-check-label" for="enable_end_date">Enable</label>
                                </div>
                                <select class="form-select @error('end_day') is-invalid @enderror" name="end_day" id="end_day" disabled>
                                    @for ($i = 1; $i <= 31; $i++)
                                        <option value="{{ $i }}" {{ old('end_day', '1') == $i ? 'selected' : '' }}>{{ $i }}</option>
                                    @endfor
                                </select>
                                <select class="form-select @error('end_month') is-invalid @enderror" name="end_month" id="end_month" disabled>
                                    @foreach (['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'] as $month)
                                        <option value="{{ $month }}" {{ old('end_month', 'January') == $month ? 'selected' : '' }}>{{ $month }}</option>
                                    @endforeach
                                </select>
                                <select class="form-select @error('end_year') is-invalid @enderror" name="end_year" id="end_year" disabled>
                                    @for ($i = 2023; $i <= 2030; $i++)
                                        <option value="{{ $i }}" {{ old('end_year', '2023') == $i ? 'selected' : '' }}>{{ $i }}</option>
                                    @endfor
                                </select>
                                <input type="time" class="form-control @error('end_time') is-invalid @enderror" name="end_time" id="end_time" value="{{ old('end_time', '00:00') }}" disabled>
                                @error('end_day')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                @error('end_month')
                    ...
                    <div id="summary2-section" style="display: none;">
                        ...
                        <div class="d-flex justify-content-between gap-2">
                            ...
                            <div class="d-flex gap-2 w-75">
                                ...
                                <button type="submit" class="btn btn-primary w-50" onclick="submitForm()">Finish</button>
                            </div>
                        </div>
                    </div>
                </form>
