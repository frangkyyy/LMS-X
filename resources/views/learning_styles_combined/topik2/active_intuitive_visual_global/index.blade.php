        ...
        <div class="card mt-3">
            <div class="card-body">
                        ...
                        <div class="card mt-4">
                            <div class="card-body">
                                ...
                                @if ($quiz->time_open && $quiz->time_close)
                                    <div class="mb-3">
                    <span class="badge bg-success me-2">
                        <strong>Opened:</strong> {{ $quiz->time_open->translatedFormat('l, d F Y, H.i A') }}
                    </span>
                                        <span class="badge bg-success">
                        <strong>Due:</strong> {{ $quiz->time_close->translatedFormat('l, d F Y, H.i A') }}
                    </span>
                                    </div>
                                @endif

                                @php
                                    $attempts = \App\Models\MDLQuizAttempts::where('quiz_id', $quiz->id)
                                        ->where('user_id', Auth::id())
                                        ->count();
                                @endphp
                ...
                <span class="badge bg-info">
                    <strong>Attempts:</strong> {{ $attempts }} of {{ $quiz->max_attempts == 0 ? 'Unlimited' : $quiz->max_attempts }}
                </span>
                                </div>
                                ...
                                @if($attempts > 0)
                                    <a href="{{ route('quiz.results', $quiz->id) }}" class="btn btn-info ms-2">
                                        View Results
                                    </a>
                                @endif
                            </div>
