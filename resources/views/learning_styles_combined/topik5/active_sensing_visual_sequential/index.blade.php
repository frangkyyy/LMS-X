    ...
    <div class="container">
            ...
            <div class="card-body">
                ...
                <div class="alert alert-info mt-2">
                    ...
                    <ul>
                        {{-- Active --}}
                        <li>
                            {{ $scores['ACT/REF']['label'] ?? 'Active' }}
                            <strong>({{ $scores['ACT/REF']['category'] ?? 'Balanced' }})</strong> -
                            {{ $descriptions['ACT/REF']['Active'][$scores['ACT/REF']['category'] ?? 'Balanced'] ?? '' }}
                        </li>

                        {{-- Sensing --}}
                        <li>
                            {{ $scores['SNS/INT']['label'] ?? 'Sensing' }}
                            <strong>({{ $scores['SNS/INT']['category'] ?? 'Balanced' }})</strong> -
                            {{ $descriptions['SNS/INT']['Sensing'][$scores['SNS/INT']['category'] ?? 'Balanced'] ?? '' }}
                        </li>
        ...
        <div class="card mt-3">
            <div class="card-body">
                ...
                {{--                @if ($section)--}}
                {{--                    <h3 class="fw-bold text-success text-center mb-3">{{ $section->name }}</h3>--}}
                {{--                @endif--}}

                {{-- === Materi Gabungan === --}}
                @if ($sensing_materials->isNotEmpty())
                    @foreach ($sensing_materials as $material)
                ...
                {{-- === Page Files Active === --}}
                @if ($page_files_active->count() > 0)
                    <div class="card mt-4">
                        <div class="card-body">

                            @foreach ($page_files_active as $page)
                                <div class="mb-4">
                                    <h5>{{ $page->name }}</h5>
                                    @if($page->description)
                ...
                @if ($active_images->count() > 0)
                    @foreach ($active_images->groupBy(fn($item) => $item->name . '||' . $item->description) as $key => $group)
                        @php
                ...
                @if ($active_videos->count() > 0)
                    <div class="mt-3">
                        ...
                        <p class="fw-bold">{{ $active_videos->first()->name ?? '-' }}</p>
                        <p class="fw-bold">{{ $active_videos->first()->description ?? '-' }}</p>

                        <div class="d-flex flex-wrap justify-content-start gap-3">
                            @foreach ($active_videos as $file)
                ...
                {{-- === PDF Materi Sensing === --}}
                @if ($sensing_files->count())
                    <div class="mt-4">
                        <h5 class="fw-bold text-primary">Materi PDF</h5>
                        @foreach ($sensing_files as $pdf)
                            <div class="d-flex flex-column mb-3">
                                <div class="d-flex align-items-center mb-2">
                                    <i class="fas fa-link text-primary mr-3"></i>
                ...
                @if ($page_files_sensing->count() > 0)
                    <div class="card mt-4">
                        <div class="card-body">

                            @foreach ($page_files_sensing as $page)
                                <div class="mb-4">
                                    <h5>{{ $page->name }}</h5>
                                    @if($page->description)
                                        <p class="mb-3">{!! nl2br(e($page->description)) !!}</p>
                ...
                @if ($sensing_image->count() > 0)
                    @foreach ($sensing_image->groupBy(fn($item) => $item->name . '||' . $item->description) as $key => $group)
                        @php
                            [$name, $description] = explode('||', $key);
                        @endphp
                        <div class="mb-3">
                ...
                @if ($sensing_video->count() > 0)
                    <div class="mt-3">
                        <h5 class="fw-bold text-primary">Video Materi</h5>
                        <p class="fw-bold">{{ $sensing_video->first()->name ?? '-' }}</p>
                        <p class="fw-bold">{{ $sensing_video->first()->description ?? '-' }}</p>

                        <div class="d-flex flex-wrap justify-content-start gap-3">
                            @foreach ($sensing_video as $file)
                                <div style="flex: 1 1 300px; max-width: 350px;">
                                    <div class="ratio ratio-16x9">
                                        <iframe src="{{ $file->file_path }}" frameborder="0" allowfullscreen></iframe>
                                    </div>
                ...
                @foreach($sensing_assignments as $assignment)
                    @php
                        $userSubmission = null;
                        $isLate = false;
                ...
                @if ($sensing_forums)
                    @foreach ($sensing_forums as $sensing_forum)
                        <div class="card mt-3">
                            <div class="card-body">
                                <h4 class="fw-bold text-primary">{{ $sensing_forum->name }}</h4>
                                <p>{{ $sensing_forum->description }}</p>

                                <hr>
                                @forelse ($sensing_forum->posts as $post)
                                    <div class="border p-3 rounded mb-3">
                                        <strong>{{ $post->user->name ?? 'User Tidak Diketahui' }}</strong>
                                        <p>{{ $post->content }}</p>
                ...
                @if ($quiz_sensings)
                    @foreach ($quiz_sensings as $quiz_sensing)
                        <div class="card mt-4">
                            <div class="card-body">
                                <h4 class="fw-bold text-primary">{{ $quiz_sensing->name }}</h4>
                                <p>{{ $quiz_sensing->description }}</p>
                                @if ($quiz_sensing->time_open && $quiz_sensing->time_close)
                                    <div class="mb-3">
                                        <span class="badge bg-success me-2">
                                            <strong>Opened:</strong>
                                            {{ \Carbon\Carbon::parse($quiz_sensing->time_open)->translatedFormat('l, d F Y, H.i A') }}
                                        </span>
                                        <span class="badge bg-success">
                                            <strong>Due:</strong>
                                            {{ \Carbon\Carbon::parse($quiz_sensing->time_close)->translatedFormat('l, d F Y, H.i A') }}
                                        </span>
                                    </div>
                                @endif
                                <a href="{{ route('quiz.show', $quiz_sensing->id) }}" class="btn btn-success">Mulai Quiz</a>
                            </div>
                        </div>
                    @endforeach
                @endif
