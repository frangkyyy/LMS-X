    ...
    <div class="container">
        ...
        <div class="card mt-3">
            <div class="card-body">
                ...
                @if ($course && $course->id)
                    @php
                        $reflectiveFiles = \App\Models\MDLFiles::where('course_id', $course->id)
                ...
                @endif
                    ...
                    @if ($imageFiles->count() > 0)
                        @foreach ($imageFiles as $imageFile)
                            <div class="mb-3">
                                <!-- Mengecek apakah file_path ada atau tidak -->
                                @if (empty($imageFile->file_path))
                                    <!-- Jika file_path kosong, hanya tampilkan name dan description -->
                                    @if (!empty($imageFile->name))
                                        <p class="text-start mt-2"><strong>{{ $imageFile->name }}</strong></p>
                                    @endif
                                    @if (!empty($imageFile->description))
                                        <p class="text-start fw-bold">{{ $imageFile->description }}</p>
                                    @endif
                                @else
                                    <!-- Menampilkan nama dan deskripsi hanya sekali di iterasi pertama -->
                                        @if (!empty($imageFile->name))
                                            <p class="text-start mt-2"><strong>{{ $imageFile->name }}</strong></p>
                                        @endif
                                        @if (!empty($imageFile->description))
                                            <p class="text-start fw-bold">{{ $imageFile->description }}</p>
                                        @endif
                                    <!-- Menampilkan gambar jika file_path ada -->
                                    <img src="{{ asset($imageFile->file_path) }}" alt="{{ $imageFile->name }}" class="img-fluid" style="max-width: 500px;" />
                                @endif
                            </div>
                        @endforeach
{{--                    @else--}}
{{--                        <p class="text-muted text-center">Tidak ada gambar untuk topik ini.</p>--}}
                ...
                @if ($course && $course->id)
                    @php
                        // Menampilkan forum dengan topik 'topik2' dan learning_style 'active'
                        $forum = \App\Models\MDLForum::where('course_id', $course->id)
                            ->where('learning_style', 'reflective')
                            ->where('topik', 'topik3')
                            ->with(['posts.user'])->first();
                    @endphp
                    @if ($forum)
                        <div class="card mt-3">
                            <div class="card-body">
