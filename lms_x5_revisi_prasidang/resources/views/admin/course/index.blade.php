...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelectorAll('.modal[id^="addParticipantModal-"]').forEach(modal => {
            ...
            // Fungsi untuk memeriksa apakah peserta sudah ada
            ...
            function addParticipant(user, courseId) {
                ...
                if (user.id && user.display && !selectedParticipants.some(p => p.id === user.id)) {
                    const role = roleSelect ? roleSelect.value : 'Student';
                    selectedParticipants.push({ id: user.id, name: user.display, role });
            ...
            function updateSelectedParticipantsList(courseId) {
                ...
                if (selectedParticipants.length === 0) {
                    ...
                    selectedParticipants.forEach(user => {
                        ...
                        li.innerHTML =
                            user.name +
                            '<div>' +
                            '<select class="form-select form-select-sm d-inline-block w-auto" onchange="updateParticipantRole(' + user.id + ', this.value, ' + courseId + ')">' +
                            '<option value="Student"' + (user.role === 'Student' ? ' selected' : '') + '>Student</option>' +
                            '<option value="Teacher"' + (user.role === 'Teacher' ? ' selected' : '') + '>Teacher</option>' +
                            '<option value="Admin"' + (user.role === 'Admin' ? ' selected' : '') + '>Admin</option>' +
                            '</select>' +
                            '<button class="btn btn-sm btn-danger ms-2 remove-participant" data-id="' + user.id + '" onclick="removeParticipant(' + user.id + ', ' + courseId + ')">' +
                ...
                participantsInput.value = JSON.stringify(selectedParticipants.map(p => ({
                    id: p.id,
                    role: p.role
                })));
            }
                ...
                searchInput.addEventListener('input', debounce(async function () {
                    ...
                    try {
                        ...
                        users.forEach(user => {
                            ...
                            if (!isSelected && user.id && user.display) {
                                item.addEventListener('click', () => {
                                    isSelecting = true;
                                    addParticipant(user, courseId);
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Kesalahan!',
                            text: 'Gagal mencari pengguna: ' + error.message,
                        });
                    }
                }, 300));
            }
                                ...
                                .then(data => {
                                    if (data.success) {
                                        Swal.fire({
                                            ...
                                            timer: 2000,
                                            willClose: () => {
                                                const addModal = bootstrap.Modal.getInstance(document.getElementById('addParticipantModal-' + courseId));
                                                addModal.hide();
                                                const participantModal = bootstrap.Modal.getInstance(document.getElementById('participantModal-' + courseId));
